...
class RunHeatmapDTUpdater(Resource):
    def get(self, id):
        # Open base.json for calculations
        try:
            f = open('./heatmapdt/basedt.json', 'r')
            data = json.load(f)
        except FileNotFoundError:
            logger.error('basedt.json not found')
            return 'basedt.json not found', 404
        ...
        with open(path, 'w') as file:
            logger.error("Absolute path: " + os.path.abspath(path))
            json.dump(arr, file, indent = 4)
            logger.error(path)
            logger.error("update dt json")
            logger.error(f"arr length: {len(arr)}")
        #sql_result = requests.put(f'http://{Config.ctrl_ip}:{Config.ctrl_port}/HeatmapDT/{id}', json = arr)
