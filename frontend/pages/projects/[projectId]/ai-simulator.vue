  ...
  <div class="ai-simulator-page">
    ...
    <div class="page-content">
      ...
      <div class="left-panel">
        ...
        <!-- RSM (Radio Slicing Management) Model 控制面板 -->
    ...
    <v-dialog v-model="showBackConfirmDialog" max-width="400" persistent>
      <v-card>
        <v-card-title>確認返回</v-card-title>
        <v-card-text>
          返回將會重置所有訓練進度，確定要繼續嗎？
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showBackConfirmDialog = false">取消</v-btn>
          <v-btn color="error" variant="elevated" @click="confirmGoBack">確定返回</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script setup lang="ts">
  ...
  const modelList = [
    ...
    { id: 'rsm', name: 'RSM' },          // Radio Slicing Management (新增)
  ...
  // RSM (Radio Slicing Management) Model 控制
  ...
  async function startPreTrain() {
    ...
    try {
      ...
      if (flResponse.success) {
        ...
        snackbar.value = {
          show: true,
          text: `NES 模型 ${nesModelSelect.value} 訓練任務已提交 (Epochs: ${trainingEpochs.value}, LR: ${trainingLearningRate.value})`,
          color: 'success'
        }
        ...
        startStatusPolling(
          ...
          (status) => {
            ...
            if (status.status === 'not in queue' && trainingStatus.value === 'running') {
              ...
              snackbar.value = {
                show: true,
                text: 'Pre-train 完成！點擊 Review 檢視結果，或點擊 Update 進入 Finetune 階段',
                color: 'success'
              }
            }
          }
      ...
      snackbar.value = {
        show: true,
        text: `訓練啟動失敗: ${flTrainingError.value || appControlError.value || '無法連接到訓練平台'}`,
        color: 'error'
      }
    }
  }
  ...
  // ========== RSM (Radio Slicing Management) Model 訓練控制 ==========

  // 開始 RSM 預訓練 (Pre-train 按鈕)
  async function startRsmPretrain() {
    ...
    try {
      ...
      if (flResponse.success) {
        ...
        snackbar.value = {
          show: true,
          text: `RSM 模型 ${rsmModelSelect.value} 訓練任務已提交`,
          color: 'success'
        }
        ...
        startStatusPolling(
          ...
          (status) => {
            ...
            if (status.status === 'not in queue' && rsmTrainingStatus.value === 'running') {
              ...
              snackbar.value = {
                show: true,
                text: 'RSM Pre-train 完成！可以進行 Enable',
                color: 'success'
              }
              stopStatusPolling()
            }
      ...
      snackbar.value = {
        show: true,
        text: `RSM 訓練啟動失敗: ${flTrainingError.value || '無法連接到訓練平台'}`,
        color: 'error'
      }
    }
  }
  ...
  async function confirmNesPretrainDone() {
    ...
    try {
      ...
      snackbar.value = {
        show: true,
        text: `Pre-train 模型 ${nesModelSelect.value} 已註冊，可以開始 Finetune`,
        color: 'success'
      }
    } catch (err) {
      ...
      snackbar.value = {
        show: true,
        text: `模型註冊失敗 (${appControlError.value || '無法連接到計算平台'})，但仍可進行 Finetune`,
        color: 'warning'
      }
    }
  }
  ...
  async function updateNesFinetuneModel() {
    ...
    try {
      ...
      snackbar.value = {
        show: true,
        text: `Finetune 模型已更新到 ${nesModelSelect.value}，可以 Enable`,
        color: 'success'
      }
    } catch (err) {
      ...
      snackbar.value = {
        show: true,
        text: `更新失敗: ${appControlError.value || '無法連接到計算平台'}`,
        color: 'error'
      }
    }
  }
  ...
  async function enableNesModel() {
    ...
    try {
      ...
      snackbar.value = {
        show: true,
        text: `NES 模型 ${nesModelSelect.value} 已啟用`,
        color: 'success'
      }
      ...
      snackbar.value = {
        show: true,
        text: `啟用失敗: ${appControlError.value || '無法連接到計算平台'}`,
        color: 'error'
      }
    }
  }
